...
def delete_old_refresh_logs(db_file, table_name, days_to_keep):
    ...
    try:
        ...
        print(f"An error occurred at delete_old_rows/deleteDBRows.py: {e}")
    finally:
        if conn:
            conn.close()
...
def delete_old_lastupdateTIme(db_file, table_name, days_to_keep):
    ...
    try:
        ...
        print(f"An error occurred at delete_old_lastupdateTIme/deleteDBRows.py:  {e}")
    finally:
        if conn:
            conn.close()
...
def delete_old_problems():
    ...
    try:
        ...
        cursor.execute(f"DELETE FROM openProblems WHERE last_update_time < ?", (threshold_str,))
        conn.commit()

    except sqlite3.Error as e:
        print(f"An error occurred at delete_old_problems/deleteDBRows.py: {e}")
    finally:
        if conn:
            conn.close()